...
<!-- TODO:REVIEW machine translation of the "Paketabhängigkeiten" section
     appended to input/translations/de/pagecontent/downloads.md — the German
     text is authoritative until Gate C signs this section off. The image
     replaces the source's external link (Referenzen.page.md) to
...
![Package dependency diagram of the MII KDS module Dokument](Paketabhaengigkeiten.png)

It is important to note that compatibility from clinical routine towards the Dokument reference can be guaranteed, but no backwards compatibility into the routine is intended. In the package dependency diagram (above), the relationships between the MII modules are shown in green.
